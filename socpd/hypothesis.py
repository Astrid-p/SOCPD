...
class Hypothesis:
    ...
    def detect_wrong_input_format(Dict_params):
        desired_dict = {'dir_params':str,
                        ...
                        'actions_to_nw': list,
                        'actions_to_self_adopt': list,
                        'actions_to_nw_neg_adopt': list,
                        'actions_to_nw_pos_adopt': list,
                        'actions_strength_of_tie': list,
                        }

        dict_params = {key.lower(): value for key, value in Dict_params.items()}
        #dict_params['env_beta'] = float(dict_params['env_beta'])
    ...
    def validate_n_read_hypotheses(cls, hypothesis_params: Dict ) -> Dict[str, pd.DataFrame]:
        ...
        # env_beta             = float(hypothesis_params['env_beta'])
        
        #---> separate params files to actions_to_self and actions_to_nw (homophily)
        # actions affecting on agents (self)
        actions_to_self      = lowerlist(hypothesis_params['actions_to_self'])
        # actions affecting on neighbor based on homophily
        actions_to_nw        = lowerlist(hypothesis_params['actions_to_nw'])    
        
        # adopt_related actions from self
        #---> separate adopt-related actions in actions_to_self
        self_adopt = lowerlist(hypothesis_params['actions_to_self_adopt'])   
         
        # adopt-related actions (neg/pos) based on homophily (status = True/1)
        #---> separate adopt-related actions in actions_to_nw, by specifying positive/negative effect of actions_to_nw 
        homo_neg_adopt = lowerlist(hypothesis_params['actions_to_nw_neg_adopt'])
        homo_pos_adopt = lowerlist(hypothesis_params['actions_to_nw_pos_adopt']) 
    
        # actions to move: both self and nw ---> To check if the given list matchs with the rest of the actions (not adopt-related actions)
        actions_to_stay = lowerlist(hypothesis_params['actions_strength_of_tie']) 
        
        # Sim validate and sim setup ________________________________________
        # SimValidate hypothesis data (with json features and desired action)
        # Simsetup cls
        ...
        if not list_equal(cls.all_possible_actions,
                         ...
                         homo_pos_adopt + \
                        actions_to_stay ):
            raise ValueError("Combination of actions deciding (adoption and strength of ties) is not aligned with 'all_possible_actions'")
        
        if not all([ifsubset(self_adopt, actions_to_self),
                    ifsubset(homo_pos_adopt + homo_neg_adopt, actions_to_nw),
                    ifsubset(actions_to_stay, cls.all_possible_actions),
                    ]):
            raise ValueError("Hypotheses aren't aligned: recheck self/network actions for adoption/ tie strength ")
        ...
        #__________________________________________________________________
        # update final hypotheses__________________________________________
        #Hypothesis.env_beta = env_beta      
        Hypothesis.self_adopt = sorted(self_adopt)
        Hypothesis.homo_neg_adopt = sorted(homo_neg_adopt)
        Hypothesis.homo_pos_adopt = sorted(homo_pos_adopt)
