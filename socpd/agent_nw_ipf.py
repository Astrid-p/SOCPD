...
from .hypothesis_nw import PARAMS_INDIVIDUAL, PARAMS_IPF_WEIGHTS, PARAMS_IPF_KEYMAPS
...
class Individual(Agent):
    ...
    def change_agent_features_by_status(self):
        ...
        - update agent's diet history by new and old status
         If agent current 2 consecutive steps are vegetarian or omnivores -> change diet history to vegetarian or omnivores '''
        if self.features[f"{self.status_var}_yes"] == 1:
            self.features[self.features.index.str.contains(pat = 'vege_history')] = [0,1]
        if self.features[f"{self.status_var}_yes"] == 0:
            self.features[self.features.index.str.contains(pat = 'vege_history')] = [1,0]
...
class Populating(Object):
    ...
    def sampling_from_ipf(self, dir_params: str, pop:int) -> pd.DataFrame:
        ...
        fpath_keymap = os.path.join(dir_params, PARAMS_IPF_KEYMAPS)
        assert os.path.isfile(fpath_weights)
        assert os.path.isfile(fpath_keymap)

        # load weight and categorical mapping dictionary
        with open(fpath_weights, 'rb') as handle:
            weight_mx = pickle.load(handle)
        with open(fpath_keymap, 'rb') as handle:
            _,cat_keymap = pickle.load(handle)

        # Randomly pick indices with weights
        weights = weight_mx.flatten()
        weights = weights / weights.sum() #re-adjust   
        sample_indices = self.nprandom.choice(np.arange(len(weights)), pop, replace=False, p=weights)

        # Convert 1D indices back to original shape
        sample_indices = np.unravel_index(sample_indices, weight_mx.shape)

        # relabeled based on the indices
        sample_data = {var: np.vectorize(cat_keymap[var].get)(sample_indices[i]) \
                   for i, var in enumerate(cat_keymap.keys())}


        return pd.DataFrame(sample_data).sample(frac=1).reset_index(drop=True) # shufle row)
    ...
    def populate_ipf(self, dir_params: str, pop:int) -> List:
        ...
        if encoded_columns.isna().any().any():
            raise ValueError("all_possible_features doesn't match ipf sample's features")
    ...
    def populate_simple(self, dir_params: str, pop:int) -> List:
        ...
        _features.insert(0, "baseline", 1)
        _features = _features.sample(frac=1).reset_index(drop=True)
